  ...
  useEffect(() => {
    ...
    getCustomerRecommendations(currentUser.customerId)
      ...
      .catch((requestError) => {
        ...
        setRecommendations([]);
        setRecommendationsError("We couldn't load your personalized suggestions.");
      })
      .finally(() => {
        if (isCurrent) {
          setRecommendationsLoading(false);
      ...
      {currentUser?.role === "CUSTOMER" ? (
        <section className="mb-8 rounded-2xl border border-primary/20 bg-secondary/40 p-6">
          ...
          {recommendationsLoading ? (
            <p className="mt-5 text-sm text-muted-foreground">Loading your suggestions…</p>
          ) : recommendationsError ? (
            <p className="mt-5 text-sm font-medium text-red-700">{recommendationsError}</p>
          ) : recommendations.length === 0 ? (
            <p className="mt-5 text-sm text-muted-foreground">
              No history-based suggestions are available yet.
            </p>
          ) : (
